...
> 细节见各日 `YYYY-MM-DD.md`。最后整理：2026-09-23（v8 压缩去重，未删任何事实性结论）

## 环境与工具链
- 同名副本多份：本机可能是 `D:\Pro\Bluedivers`，规则里 MCP 钉 `E:\Bluedivers`；另一项目 `D:\Project\RTSClient`。**实例 hash 随项目路径变化**，不可跨机照抄
...
- 只需"启用/禁用物体"用 `List<{GameObject,bool}>`；仅需"任意组件任意方法"才用 UnityEvent；共享+需引用实例层级→间接引用（key/相对路径+运行时解析+编辑器校验）
...
## 场景加载与事件时序（易踩）
- `ResSvc.AsyncLoadScene(name, cb, ...)` 用 `LoadSceneMode.Single`，且 `completed` 只置标记，**回调被 `DelayedInvoke` 延后一帧** → **场景内组件的 `Start` 一定早于回调里 `new GameObject().AddComponent<BattleManager>()`**
...
- 同理：`Awake` 订阅的场景组件（`NightLight`）没问题；`Start`/回调之后才创建的（`BattleManager`、战备 UI）必须"补初始状态"
...
- ⚠⚠ **`authorizeCounter` 是裸计数、无下限**（`+= state?1:-1`，`IsAuthorize => !cfg.authorize || counter > 0`）⇒ **"白天"=计数 0 的自然状态，绝不能在白天做 `-1`**，否则战备永久锁死。详见「战备系统」
...
## 地形/植被/破坏
- 地形数据 `Assets/Art/TerrainData/MainMap.asset`（**二进制**读不了）；地形**每局运行时重建**：`BattleManager.InitTerrain` 先按 `taskCfg` 改 `heightmapResolution=MapRes+1`/`alphamapResolution=MapRes`(512/1024)/`size=(MapSize,MapHeight,MapSize)` → `MapRoot.Init(true)` → `SetTextures` → `ApplyFractalNoiseToTerrain`。**→ 场景里预摆的物体 Y 坐标每局都失效**
- `GenerateNoiseTerrain` 顺序：基础地形→后处理→材质→分块提交高度图(128)→alphamap→`PlaceRockCovers`→`SpawnVegetation`(石块/树)→`SetTreeInstances`→`TreeDestructor.Rebuild`→`TerrainDetailEraser.Rebuild`→`RebuildTreeColliders`→`SpawnDetails`(草)→NavMesh。插"落位/撒物"放 660-710（NavMesh 之前）
- 贴图注入 `SetTextures`，层索引 0 草/1 沙/3 巢穴(`CampData_SO.NestTerrainItem` 覆盖)/4 岩；实测 6 层（Desert_Crater/Plains/Hollow/Nest/Slopes/City_Hollow）。`ApplyTextures` 实际**硬编码**贴图索引 1/2/3/4（`TerrainPresetData` 里曾有的 `sand/grass/rockLayerIndex` 是死字段，2026-09-23 已删）
- **细节（草）统一走 `SpawnDetails`**：遍历 `detailPrototypes` 全部原型（0..protoCount-1），按 `preset.detailDensity` 逐格概率 + `detailMin/MaxSlope`/`detailMin/MaxHeight` 约束生成，最后逐层 `SetDetailLayer`。**2026-09-23 起不再区分"花"**（`detailFlowerDensity` 已删）
- **原型与区间全部来自 `MapData_SO`（2026-09-23 定稿）**：`ApplyFractalNoiseToTerrain(TerrainType, GameObject[] stone, GameObject[] tree, GameObject[] detail, float treeMul, float rockCoverMul)` → `ApplyMapPrototypes(stone, tree, detail, out stoneRange, out treeRange)`。地形树原型 = **石块 + 树（石块在前）**，细节 = `detailPrototypes`；**区间由数组长度推导**（`VegetationSpawnData.prototypeRange` 已删）：石块 `(0,stone-1)`、树 `(stone, stone+tree-1)`，某类为 0 → 空区间（`SpawnVegetation` 有"空区间即跳过"守卫）。推导结果同步写入 `TerrainClearer.Rock/TreePrototypeRange`（清除侧用同一套区间，否则清石块会清到树上）。`terrainData.RefreshPrototypes()` 必须调（否则 prototypeIndex 错位）。
- ⚠⚠ **换 `treePrototypes` 前必须先清树实例**：数组变短时 Unity 会立刻剔除引用越界索引的旧实例并刷 `Tree removed: invalid prototype N`（错误堆栈指向赋值那一行）→ 先 `terrainData.SetTreeInstances(Array.Empty<TreeInstance>(), false)` 再赋值
- ⚠ **无兜底**：MapData 的 `stone/treePrototypes` 为空 = 该图**不生成石块与树**（`detailPrototypes` 空 = 不生成草）。`MD_Millennium` 三组空 → 变成光地
- **地形资产原型实测（2026-09-23）**：`treePrototypes=11`（0-6=`Rock1LOD_grup1/2/3`+`RockLod1C/E`+`RockLOD4B`+`RockLOD6C` 石块、7-10=`Tree_01/23/24/25_Spring` 树）、`treeInstances=2775`、`detailPrototypes=7`（全是 `Grass_15/17/18/19/20/21/22`，**本来就没有花**）。旧代码 `validGrass{0,1,2,9,10,11}`/`validFlowers{3..8}` 过滤后实际只剩 `{0,1,2}`+`{3..6}` → 旧"花"其实是 Grass_19~22
- **Tree Prototypes 索引约定：石块在前连续段、树紧随其后**；`TerrainPresetData` 用 `rockSpawn`/`treeSpawn` 两份 `VegetationSpawnData`（`probability/minSlope/maxSlope/minHeight/maxHeight`）。概率（石/树）：沙漠 .0012/.0002、高原 .0015/.0008、雨林 .0008/.003、丘陵 .0025/.0015、盆地 .002/.001、平原 .0012/.0006、山地 .0025/.0003。另含 `rockCover`(`RockCoverSpawnData`)+`detail*` 字段
- `SpawnVegetation`：先 `Random.value >= rate` 早退，再查高度/坡度；`probability`=**每格（高度图索引）**概率，格边长=`MapSize/mapRes`；`widthScale/heightScale=Random.Range(0.7,1.5)`
- 覆盖模式：`_overridePreset` 勾上后 `_rockProbability`/`treeProbability` 生效（**<0 = 沿用预设**，默认 -1）；**地图级倍率** `MapData_SO.TreeSpawnMultiplier`/`RockCoverMultiplier`（默认 1；0=不长树/不放悬崖）→ `InitTerrain` 传 `ApplyFractalNoiseToTerrain(type, treeMul, rockMul)`，**只作用于树密度与悬崖数量**（石块/草不吃）
- ⚠ **碰撞体/隐形墙**：草永远无碰撞体；树/石碰撞体 = 原型 prefab 上的 **CapsuleCollider**（Unity 只认 Capsule）+ `TerrainCollider.m_EnableTreeColliders`（全局开关，运行时生成）；碰撞链路**不看 LODGroup**。实测树原型经 `widthScale/heightScale`(0.7~1.5) 放大 ⇒ **半径 7.4m 的隐形巨胶囊**，属于 `TerrainCollider`、**不是 GameObject**（场景视图找不到，只能 Physics Debug）。表现=跳不起来/子弹打空墙；`HaveObstacle()` 不做前探 ⇒ 不报"有障碍物"。诊断=从"脚底+1.8m"向上射线。修复=删掉原型上除树干胶囊外所有 Collider。别用 `SerializedObject` 读 `m_EnableTreeColliders`（不可信）
- ⚠ 树原型**必须带 `LODGroup`** 才会被地形渲染（无 → 判为 Tree Editor 树 → 项目 shader 不满足其配套 → 基本不渲染，但仍产生隐形胶囊）。工具 `Assets/Editor/TerrainTreePrototypePreparer.cs`（`Tools/地形树原型预处理`）批量补「只有 LOD0」的 LODGroup；**已对 Pandazole `Tree/Prefabs` 下 99 个 prefab 全量执行成功**
- ⚠⚠ **树碰撞体不随 `SetTreeInstances` 更新**（旧位置仍挡、新位置没碰撞体，`terrain.Flush()` 无效）：必须显式 `TerrainUtils.RebuildTreeColliders(terrain)`（切一次 `enabled`，唯一廉价有效手段，代价=重建整张地形碰撞体，勿频繁调）→ 已接入 `TerrainClearer`（`CommitVersion` 门控 + 同帧去重 + `ColliderSyncInterval=0.5f`）
- 悬崖（地形覆盖石）落点必须**按网格最低点对齐**：`Instantiate` 后量 `GetRendererMinY(go) - go.transform.position.y`，再把 pivot 放到 `地表 - sinkDepth - 该偏移`
- `TerrainData` **无 `RemoveTreeInstance`**；只能 `SetTreeInstance(s)`（单实例版不可改 `position`/`prototypeIndex`，整表可）；`TreeInstance.rotation`（X-Z 弧度）可赋值；位置是**归一化坐标(0~1)**。`treePrototypes`/`detailPrototypes` 是 public get/set，脚本可整体赋回（getter 返回副本）；改完必须 `RefreshPrototypes()` + 重写 `treeInstances`。**2026-09-23 起运行时赋值这四处原型的就是 `GenerateNoiseTerrain.ApplyMapPrototypes`**（其余仍写在地形资产里）
- 树销毁 `08Map/TreeDestructor.cs`：索引→世界坐标表 + 圆柱射线 + 按帧合并提交（限流 4/s）；销毁=实例 scale 置 0（索引不变、可 `RestoreAll`），`SetTreeInstances(arr,false)`（**必须 false**）+`terrain.Flush()`。两套语义：① 摧毁 `DestroyInRadius` 走白名单（空=全放行）② 清除 `ClearInRadius/ClearInRectXZ` 按原型区间（忽略白名单，-1=不限）。白名单落点 `GenerateNoiseTerrain._destructibleTreePrototypes`
- 细节（草）擦除 `08Map/TerrainDetailEraser.cs`：同款"入队+按帧合并提交(限流 8/s)"，`Rebuild(Terrain)` 自动挂载；逐层 `GetDetailLayer→清零→SetDetailLayer`（rect 先 Clamp；**Terrain.position 是角点**）
- **地表物清除统一门面 `08Map/TerrainClearer.cs`**（静态 + `[Flags] TerrainClearTarget{Tree,Rock,Vegetation,Detail,VegetationAndDetail,Cover,All}`）：`ClearInRadius/ClearInRectXZ`（清除）/`DestroyInRadius`（摧毁，爆炸用）/`MarkTerrainChanged()`/`Flush()`/`Rebuild(terrain,destructibleProtos)`（⚠ 含 `RockCoverDestructor.Rebuild`=ClearAll，**只能在放置覆盖物之前调**）。请求只入队 → 帧末隐藏驱动器（`[RuntimeInitializeOnLoadMethod]` 自建）统一下发 → 昂贵操作按 `RefreshInterval`(0.25s) 合并，**顺序必须先生效再重烘**
- **弹坑/地形破坏链路**：伤害总入口 `FpsHelper.Hit`（`destructe>0` → 擦雪+`ModifyHeightMap(...,refresh:false)`+`MarkTerrainChanged()`）；`ModifyTerrain`（弹坑=圆按 `outerRadius`；附加地形=角点+半尺寸矩形 **+`transitionDistance` 过渡带**；清除范围 `clearVegetationRange`(默认(7,10)=树)+`clearRockRange`(默认(0,6)=石块，-1=不限)——**石块必须一起清**）挂在 **31 个任务/兴趣点 prefab** 上；实战兜底 = `MissionController.InitializeAsync` 末尾 `AsyncRefresh(true)`，但顺序不保证
- 弹坑三件事（改高度图+清树/石+清草）最贵的四步：① `ModifyHeightMap` 的 `SetHeightsDelayLOD`+**`SyncHeightmap`**+`ModifyAlphaMap` ② 树实例整表提交 ③ 树碰撞体重建 ④ NavMesh `UpdateNavMesh`。优化方向：写 patch 与 Sync 分离统一、已清除区域去重、分级限流（树/草 0.1~0.25s、碰撞体 0.5s、NavMesh 1s+）、NavMesh 阈值化/`NavMeshObstacle(carve)`、树实例真删、关 `Enable Tree Colliders` 改 Collider 池
- **实测（2026-09-21 Play 中）**：`SetTreeInstances(3650)`=0.08ms、`terrain.Flush`=0.69ms、**`RebuildTreeColliders`=10.75ms**、高度图写+Sync=2~5ms、`UpdateNavMesh`=5.6~8.8ms(仅发起)。计时入口：`TerrainClearer.LogTiming/TimingSamples/ResetTimingStats`、`TerrainUtils.LastHeightReadMs/LastHeightWriteMs/HeightModifyCount/ResetHeightTimingStats`
- `TerrainUtils.AreaCircles`（纯数据"地表占地圆"，`00Tools/Test/TerrainMainUtils.cs`）：唯一写入者=`08Map/RockCoverDestructor`；生产者=`GenerateNoiseTerrain.PlaceRockCovers`；消费者=`MissionController` 与 `ModifyTerrain`。换地形自动清空
- `TerrainUtils`（partial static，`Main`=TerrainUtils.Main）：`WSToHeight(pos)`（世界→地形高度，**prefab 落位贴地用它**）、`WRToHR/WRToAR/ARToHR`、`ModifyHeightMap`、`AdditionTerrain`、`Refresh/AsyncRefresh(refreshNav)`、`RebuildTreeColliders`
- 层掩码（`GameContract.LayerDefinition`）：子弹 `HittableLayers=73`、高速 `601`、玩家移动 `MoveableLayers=8265`、`UnitSeeLayers=16393`、`AirWallLayers=8192`。Terrain 在 Ground(3)、`preserveTreePrototypeLayers=False`；`Physics.queriesHitTriggers=True`
- **NavMesh**：MapRoot 上 `NavMeshSurface`，`collectObjects=Volume(1)`、`m_UseGeometry=0`(RenderMeshes)、`LayerMask=8`(Ground) → **树/石块（Default 层）都不进 NavMesh**。想让运行时摆的石头挡 AI：放 Ground 层或生成后再烘
- "看不见的碰撞体"三类：① 地形树/石碰撞体（非 GameObject）② `MapRoot.CreatAirWall()` 运行时 18 个边界墙 ③ 建筑隐形代理（`OilPlane`/`Turret_Mission_Machine` 的 `PlaneA1..F`/`Spine2`/`StairsA1..C2`/`GunArmor`，Default 层无 Renderer）+ `EntityRoot` 的 `KeyScreen`
...
- 背景层物体（`Environment/SkyboxLayer`：星星/月亮/云，尺度 13000~15000，靠 `ZClip Off` 越过远裁剪面）：队列 <2500 时在不透明 Pass 画 → 被天空盒擦掉；队列 Transparent 时能看到但不吃雾
- **解法（已落地）**：`SkyboxLayer.shader` Pass 改自定义 `LightMode = "SkyboxLayerBeforeFog"`，`Rendering/SkyboxLayerBeforeFogRendererFeature.cs` 在 **445** 绘制并注册。要点：① pass 必须 `CoreUtils.SetRenderTarget(cmd, colorHandle, depthHandle)` 同时绑深度 ② FilteringSettings 用 `RenderQueueRange.transparent` + `SortingCriteria.CommonTransparent`
- 同款先例 `WarpingBeforeFogRendererFeature`(445)：**自定义 LightMode 的代价 = 特性被禁用/移除后该特效完全不显示**
- URP 资产：`Assets/Setting/New Universal Render Pipeline Asset.asset` → `..._Renderer.asset`；已挂 5 个 Feature：Outline(300)/Snow(300)/SkyboxLayerBeforeFog(445)/WarpingBeforeFog(445)/FullScreenFog(450)
...
- ToonLit 5 pass（ForwardLit/Outline/ShadowCaster/DepthOnly/DepthNormalsOnly）**共用 `ToonLit_Shared.hlsl`**（7 个 shader 共用：`ToonLit`/`_Colour`/`_Face`/`_Hair`/`_MouthEye`/`_Scene`/`_Stone`）；`GetFinalBaseColor`(376) 是 albedo 唯一入口；透明完全靠 `clip()`
- 给 shared 加东西三条边界：① 宏只写在**目标 shader 自己的 `HLSLINCLUDE`** ② shared 里只**新增** `#ifdef` 块 ③ 新数据只走**全局 uniform**，不进 Properties/CBUFFER，也别动 `Varyings`；**宏要按 pass 精确控制**
...
- 从 Properties 删**被代码读取**的属性 = CBUFFER 取 0；`_BaseMap_ST`/`_BlendingMap_ST` 仍被 uv 使用不能删；`ToonLit.shader`(main) 挂 `CustomEditor "ToonLitMainShaderGUI"`（不判空）→ 别在 main 上删属性
...
- **昼夜解锁照明战备**：`BattleManager.OnDatSwitch(isNoon)` 暂存 → `ApplyDaySwitch()` → `Authorize(LampTowerId/IlluminatorId, isNight)`，**只在翻转时动计数**（`_nightAuthorized` 去重），白天什么都不做；开局事件会漏 → `GlobalEventSub.LastDaySwitchIsNoon` 缓存 + `ApplyInitDaySwitch()` 补发
...
- 地雷 `02Game/Gameplay/Projectile/DeployableMine.cs`（`Mine`/`RobotMine`/`spore`）：`Actor`/`Health`/`LimitedLife` 三件套；触发链路 = 自动命中/被摧毁/外部调**幂等的** `TriggerExplosion()` → `OnTriggered` → `ExplosionDelay` 秒协程 → `DoExplosion()`（伤害 → `OnExploded`（**在 `VFXManager.Release` 之前**派发）→ 回池）；`LimitedLife` 到寿时"已触发则立即引爆、未触发才 `Tool.Destroy`"。⚠ 新序列化字段缺 YAML 行时用 C# 初始化器默认值
...
- 昼夜：`DayNightBrain` + Modules（`TimeProgressionModule`/`CelestialRotationModule`/`CelestialVisualsModule`/`EnvironmentLightingModule`/`DayNightTriggerModule`）；预制体 `Day-Night-Manager.prefab`，**Utnapishitim（桥）与 TestScene（战斗）各一个实例**；天空盒运行时建副本写值（`GetSkyboxForWrite`）。旧 `Effect/DayNightCycle.cs` 已不参与玩法场景
...
## 编辑器扩展
- 装饰特性一律 `DecoratorDrawer`；需读 propertyPath/serializedObject 才用 `PropertyDrawer`
- `InlineFieldDrawer` 是 `[Singleline]` 内联的**唯一实现**；`EditorOverride`（全局 fallback Inspector）**被专属 `[CustomEditor]` 完全顶掉**（全仓仅 `SoundGroup_SOEditor`、`AirdropData_SOEditor`）
...
- 本机双实例：`Bluedivers@3d9f2357`(port 6400, `E:/Bluedivers/Assets`) / `RTSClient@6365de15`(6401, `D:/Project/RTSClient`)。实例 hash **随项目路径变化**，先读 `mcpforunity://instances`；活动实例只存内存，重开会话必须重设 `set_active_instance`；**写操作前核对 `projectRoot`**
...
## 第三方素材包
- Pandazole 包（`Assets/Pandazole_Ultimate_Pack`）99 个 fbx 经 `.fbx.meta` 的 `externalObjects` 全部重映射到**共享材质** `PandaMat.mat`，其 shader = 项目自己的 `Assets/Shader/ToonLit/Main/ToonLit_Scene.shader`（URP 下正常，不会洋红）
- ⚠⚠ **Pandazole 包的 git 状态与磁盘长期不一致**：磁盘是 `Pandazole Nature Environment Pack/Tree/{Models,Prefabs}` + `Details`（按 Desert/Forest/… 分类），git 索引里却是**顶层 `Models/`(518 条)+`Prefabs/` 扁平结构**，`Tree/`、`Details/` **全为 untracked（612 个文件）** → 工作区常驻 **1180 个 `D` + 612 个 `??`**。⇒ **该包内资产的改动无法用 git 回滚**，批量改资产前先备份；`PrefabUtility.SaveAsPrefabAsset` 会重建文件（CreationTime 更新），但不改 fbx
...
- 排查视觉问题先看**实际材质/资产属性值**；也要先确认物体落在哪个 Pass/队列
...
- ⚠ **别什么都往 `BattleEventSub` 塞**（用户明确要求）：一方已能拿到另一方实例时就订阅**实例事件**；全局事件层只留给真广播；跨实例状态也别用 `static`。**例外**：需要"记住最后一次值给晚订阅者"的初始状态广播正是全局事件层的合理用法
- 新增脚本尽量用**运行时自动挂载/自建组件**接入，避免改场景/prefab YAML；需要手改 prefab 时明确告知用户去点；改 URP renderer 资产可走 MCP `SerializedObject`
