...
> 详细复盘见各日 `YYYY-MM-DD.md`，本文件只留跨会话结论。最后整理：2026-09-17

## 环境与工具链
- 双机：`D:\Pro\Bluedivers`（Administrator）/ `E:\Bluedivers`（admin）。绝对路径、MCP 实例 hash、uvx 路径不可跨机照抄；另装有 `D:\Project\RTSClient`、`E:\rtsclient`
- unity-mcp 多工程共用：客户端 `mcp.json` 不改，但每工程各装一份包 + 各切 Stdio；**≥2 实例且未钉选时 server 直接报错**（上游 #1023）→ 用 `.codebuddy/rules/UnityMCP_多实例路由.md` 钉死自己的实例（`Bluedivers@3d9f2357`@6400 / `RTSClient@6365de15`@6401）；活动实例只存内存，重开会话必须重设；任何写操作前先核对 `projectRoot == E:/Bluedivers`
- Unity 2022.3.62f3；URP 14.0.12；C# 9 / .NET Std 2.1；TMP 3.0.9；Navigation 1.1.6；脚本实际 UTF-8 无 BOM；单机 PvE demo（Unity FPS Sample 二次开发，命名空间 `Unity.FPS.*`）
...
- 工具坑：`search_content` glob 不支持 `*.{a,b}`，须分开写；PowerShell 管道里 `Get-Content` 不带 `-Encoding` 会被拦
...
- asmdef 仅 14 个且都在子目录；`04UI/`、`02Data/`、`01Manager/` 根脚本属 **Assembly-CSharp**（故 `WndManager` 可直接引用 `VehicleWnd`/`AirdropConfigWnd`）；asmdef 不可被 Assembly-CSharp 反向引用（`00_Utils`/`08_Map`/`04_UI`/`Effect/EffectComp` 调不到 `SnowController`/`BattleManager`）
...
- 程序集存在 **01Manager↔02Game↔02Data 三角循环**（02Data 引 `FpsGame.Mission`）→ 拆解须先断开 02Data→02Game
...
- SO **不能持有 prefab/场景实例引用**：子物体/挂点留在 prefab 的 `[SerializeField]`；SO 只放与实例无关项（音效、粒子、材质、颜色、数值）。判定「改这条希望同类单位一起吃？」是→SO；语义不同时**叠加**而非覆盖
...
### 组件存活 / 回收（易踩）
- `VFXManager.Release(GameObject)` **只认根物体**上的 `ParticleSystem`(Stop) 或 `LimitedLife`(allowRelease=true)，两者皆无＝纯空转；`ProjectileBase` 走另一个重载（回池 + `Template`）
...
- ⚠ 要**连 Awake 都不执行**：`enabled=false` 无效（Awake 只看 `activeInHierarchy`），必须在**未激活挂点**下实例化 → 移除逻辑组件 → 再激活（`Destroy` 帧末生效，需等一帧；同帧用 `DestroyImmediate`）。现成实现 `AirdropConfigWnd.ShowModel`
- `FitModelScale`：相机视野半高/半宽 + 包围盒 XZ 半对角线 / Y 半高求缩放（**双向**，小体型也放大），包围盒中心对齐**相机视线中心**（`focus = camPos + fwd*dot(root-camPos,fwd)`）；朝向走 `_modelBaseEuler`（默认 180）+ `ApplyModelRotation()`
- ⚠ 量包围盒四个前提：① `Renderer.bounds` 未激活时为 0 ② 带 Animator 必须先算姿势（`animator.Update(0f)` 再等一帧，绑定/显示姿势可差 1.7 米）③ 蒙皮 bounds 随骨骼姿势变 ④ 量前把模型自身旋转临时归零（炮台自带 45° 会让水平尺寸虚高四成）
- 保留预制体自身旋转/缩放（`_modelBaseScale`），默认朝向只加在挂点上 → 世界旋转 = 挂点(默认+拖动) ∘ 预制体自身旋转
- ⚠ **不能直接 Encapsulate 全部 Renderer**：战备预制体混着 ① 零体积包围盒的未播放粒子（会把远处那点并进来，模型被缩成 0.002）② 几百米 LineRenderer ③ `BombRange`/`EvaRange`/`Range_Sphere`/`TaskShowRange`/`DistGround*`/`Decal*` 范围网格。过滤顺序：画不出来的（`!enabled||!activeInHierarchy`）→ 特效类（Particle/Line/Trail，仅在还有实体网格时）→ 材质名关键字（`RangeMaterialKeys`）→ 零体积；全模型只剩特效才退回按全部量

## 编辑器扩展
- 装饰特性一律 `DecoratorDrawer`；需读 propertyPath/serializedObject 才用 `PropertyDrawer`（数组场景 `.Array.data[` 回退普通绘制）
- `InlineFieldDrawer`（`Assets/Editor/Drawer/`）＝`[Singleline]` 单行内联的**唯一实现**（Layout 版 `DrawInlineListLayout/DrawInlineObjectLayout/DrawInlineList/DrawDecorators`；Rect 版 `DrawInlineListRect/GetInlineListHeight/DrawElementRow/DrawFooterButtons/DrawAddButton`，`InlineListStyle` 可插 `ChildLabelProvider`/`ChildDrawer`；含 `ResolvePropertyType/ResolveField/GetFieldLabel`）。要单行内联就调它
...
- `[DisplayField]`（`00Attribute/CustomAttribute.cs` + `CustomLabelDrawer.DisplayFieldDrawer`）：默认编辑期不画、运行期只读；只对已序列化字段生效（私有须配 `[SerializeField]`）；`readonly`/Dictionary 无效；白名单 `IsSupportedType`＝Integer/Float/Boolean/String/ObjectReference/**Color/Vector2/Vector3**，不在白名单不画也不占位；绘制/高度共用 `ShouldDraw`（`isPlaying ? run : editor`）
...
- ⚠ `labels` 是后加字段：除 `ADSO_R_Railgun`（`labels: 0`）外其余资产 YAML 里**没有该行**（=0），需逐个填
...
- UI：`AirdropWnd`（HUD）、`AirdropConfigWnd`（购买/偏好；列表分组由静态 `GroupRules` 驱动：轰炸→轨道打击/凤鹰空袭、装备→战术背包/无人机/支援武器、炮台→地雷发射器/哨戒炮、载具整组、补给不显示；首个命中即归类，组内偏好优先；表头第二文本＝已拥有/总数）、`ArmamentWnd`；`ArmamentButton.prefab` 子1=偏好标记
...
- ToonLit `_ShadowMapColor`＝**暗面里该光源的乘数**；未写 CBUFFER 的属性＝死属性；新增 CBUFFER 字段须同步 Properties
- `_MAIN_LIGHT_SHADOWS` 管**接收**；URP 投影剔除走 `DrawShadows`，与 Render Queue 无关；**材质不投影头号原因**＝`disabledShaderPasses: - SHADOWCASTER`
...
- 两种：`SimpleDecal`（fog 恒开）、`SimpleDecal_Colour`（三平面视角颜色，有 `_UnityFogEnable` 开关）；两者都用 cube 罩住目标 + `ZTest Off`/`ZWrite off`/`Queue Transparent-499`，frag 里按 `_CameraDepthTexture` 重建场景位置并 `clip` 出立方体体积
- ⚠ **`_Cull` 必须是 Front(1) 或 Off(0)，绝不能 Back(2)**：相机进到 cube 内部时，所有面都成了背面 → Cull Back 全剔掉、贴花整个消失（Off/Front 都能画，Front 每像素只画一次，Off 会双面叠两次颜色）。`PylonPower.mat` 就是被设成 2 才出现"进 box 就看不见"
- ⚠ **雾与混合模式必须匹配**：`MixFog()` 是"把颜色往雾色插值"，适配 `SrcAlpha/OneMinusSrcAlpha`（`F(1-i)` 项被 `A+(1-A)` 抵消，雾既不增也不减）；但加法式混合（`_DecalDstBlend=DstAlpha(7)`，背景被完整保留）下，贴花在纹理全黑处也会**额外叠加一份雾色** → 整个足迹变成肉眼可见的方块（只在开雾时出现）。`SimpleDecal` 已加 `[Toggle(_DecalAdditiveFog)]` 开关（默认 0，仅 `PylonPower.mat` 开启）：开启后 `col.rgb = MixFog(col.rgb,f) - MixFog(half3(0,0,0),f)`（＝乘无雾强度，加法层的正确雾行为）
- ⚠ `col.rgb *= col.a` 预乘 + 硬件再乘 `SrcAlpha` ＝ alpha²：alpha=1 无影响，alpha<1 时雾只按 alpha² 生效 → 同源色块。要么删掉这行，要么改 `Blend One OneMinusSrcAlpha`
- 顶点雾（物体自身几何）＝`ComputeFogFactor(positionHCS.z)` + `MixFog()`；屏幕空间/投影式（贴花、抓屏、全屏 quad）＝`ComputeFogFactorZ0ToFar(max(sceneDepthVS-_ProjectionParams.y,0))`

### 自定义 UI Shader（`Assets/Shader/UIImageChannelMix.shader` 等）
- `Mask` 与 `RectMask2D` 是两套机制：前者走模板缓冲（`_Stencil*` + Stencil 块）；后者**不用模板**，靠 `MaskableGraphic.SetClipRect` → `CanvasRenderer.EnableRectClipping(rect)` 注入 `_ClipRect` 并开 `UNITY_UI_CLIP_RECT`，shader 自己软裁剪
...
## MCP for Unity
- 本地嵌入包 `Packages/com.coplaydev.unity-mcp`（10.2.1-beta.6，已入库，**含汉化**）；操作手册＝skill `.codebuddy/skills/unity-mcp/`；**动写操作前先 git commit**；`execute_code` 只有 CodeDom（无 Roslyn，即 C# 6 语法）——调 `Material.SetKeyword(ref LocalKeyword, true)` / `IsKeywordEnabled(ref k)` 时**必须显式写 `ref`**；改材质关键字/属性后用 `EditorUtility.SetDirty` + `AssetDatabase.SaveAssets` 落盘（本工程 toggle 的局部关键字名＝**原名**，无 `_ON` 后缀）
- Unity 侧 Transport 必须设 **Stdio**；桥监听 `127.0.0.1:6400`；状态文件 `%USERPROFILE%\.unity-mcp\unity-mcp-status-<hash>.json`；实例 hash 由项目绝对路径算出
...
- 改完脚本：`validate_script` → `refresh_unity`（`mode=if_dirty, compile=request`）→ 桥短暂失联属域重载，等 ~10s 再 `read_console`
- 查"场景里某个 shader/组件到底挂了谁"用 `execute_code` 遍历 `FindObjectsOfType<Renderer>()` 打印路径+材质属性，比肉眼开窗口快
...
- 排查视觉问题先看**实际材质/资产的属性值**（`_Cull`/blend/纹理格式），不要只看 shader 默认值：材质可以覆盖成和默认完全不同的语义
