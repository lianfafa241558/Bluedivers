...
namespace Pixeye.Unity
{
    public partial class EditorOverride
    {
        ...
        private void DrawInlineObject(SerializedProperty prop)
        {
            ...
            // 解析内联对象的类型（用于读取 InspectorName）
            Type inlineFieldType = null;
            {
                var targetType = target.GetType();
                var fieldInfo = targetType.GetField(prop.name,
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (fieldInfo != null)
                    inlineFieldType = fieldInfo.FieldType;
            }

            // 用 EditorGUILayout 自动布局
            ...
            foreach (var child in childProps)
            {
                ...
                string shortLabel;
                if (inlineFieldType != null)
                {
                    var cf = inlineFieldType.GetField(child.name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                    if (cf != null)
                    {
                        var attr = Attribute.GetCustomAttribute(cf, typeof(InspectorNameAttribute)) as InspectorNameAttribute;
                ...
                // 标签和字段分开绘制，避免 PrefixLabel 两行回退
                float shortLabelWidth = EditorStyles.miniLabel.CalcSize(new GUIContent(shortLabel)).x + 4;
                ...
                list = new UnityEditorInternal.ReorderableList(prop.serializedObject, prop, true, false, true, true)
                {
                    ...
                    drawElementCallback = (rect, index, isActive, isFocused) =>
                    {
                        ...
                        if (childProps.Count > 0)
                        {
                            ...
                            // 标签和字段分开绘制，避免 EditorGUI.PrefixLabel 导致两行布局
                            float inlineX = rect.x + labelWidth;
                            float inlineWidth = rect.width - labelWidth;
                            float childTotalWidth = inlineWidth / childProps.Count;
                            float originalLabelWidth = EditorGUIUtility.labelWidth;

                            // 解析元素类型（用于读取 InspectorName）
                            ...
                            for (int ci = 0; ci < childProps.Count; ci++)
                            {
                                var cp = childProps[ci];
                                if (cp.name == "m_Script") continue;

                                string shortLabel;
                                if (elementFieldType != null)
                                {
                                    var childField = elementFieldType.GetField(cp.name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                                    if (childField != null)
                                    {
                                        var attr = Attribute.GetCustomAttribute(childField, typeof(InspectorNameAttribute)) as InspectorNameAttribute;
                                        shortLabel = attr != null ? attr.displayName : cp.displayName;
                                    }
                                    else
                                    {
                                        shortLabel = cp.displayName;
                                    }
                                }
                                else
                                {
                                ...
                                float labelW = EditorStyles.label.CalcSize(new GUIContent(shortLabel)).x + 4;

                                Rect lblRect = new Rect(inlineX, rect.y, labelW, rect.height);
                                Rect fldRect = new Rect(inlineX + labelW, rect.y, childTotalWidth - labelW, rect.height);
                                EditorGUI.LabelField(lblRect, shortLabel);
                                EditorGUI.PropertyField(fldRect, cp, GUIContent.none);

                                inlineX += childTotalWidth;
                            }
