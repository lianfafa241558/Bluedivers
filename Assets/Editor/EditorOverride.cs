...
namespace Pixeye.Unity
{
    ...
    public partial class EditorOverride : Editor
    {
        ...
        bool initialized;
        Dictionary<string, string> listLabels = new Dictionary<string, string>();
        ...
        public override void OnInspectorGUI()
        {
            ...
            void Body()
            {
                ...
                for (var i = 1; i < props.Count; i++)
                {
                    ...
                    {
                        // 如果是数组/List，使用自定义数组绘制（保留原生样式，元素内联）
                        if (prop.isArray)
                        {
            ...
            void Foldout(CacheFoldProp cache)
            {
                ...
                void Child(int i)
                {
                    string label = listLabels.TryGetValue(cache.props[i].name, out var cl) ? cl : ObjectNames.NicifyVariableName(cache.props[i].name);
            ...
            void Setup()
            {
                ...
                if (!initialized)
                {
                    ...
                    foreach (var field in objectFields)
                    {
                        ...
                        if (attr != null)
                        {
                            listLabels[field.Name] = attr.displayName;
                        }
                    }
