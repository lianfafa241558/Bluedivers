...
namespace Unity.FPS.EditorExt
{
    ...
    public class MiniProfiler : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("您必须退出播放模式才能使用此功能",
                    MessageType.Warning);
            }
            else
            {
            ...
            else
            {
                ...
                if (s_CellDatas.Count > 0)
                {
                    ...
                    m_CellTransparency = EditorGUILayout.Slider("单元格透明度", m_CellTransparency, 0f, 1f);
                    ...
                    m_CellThreshold = EditorGUILayout.Slider("单元格显示阈值", m_CellThreshold, 0f, 1f);
        ...
        void DisplayTips()
        {
            ...
            EditorGUILayout.HelpBox("将刚体（物理对象）添加到级别可能会影响性能",
                MessageType.None);
            EditorGUILayout.HelpBox(
                "从顶部菜单栏（窗口>分析>探查器）中打开探查器窗口，以查看有关游戏性能的详细信息",
                MessageType.None);
        }
        ...
        void AnalyzeLevel()
        {
            ...
            foreach (MeshFilter mf in meshFilters)
            {
                ...
                if (!willBeCombined)
                {
                    if (!(mf.GetComponentInParent<PlayerController>() ||
                          mf.GetComponentInParent<EnemyController>() ||
                          //mf.GetComponentInParent<Pickup>() ||
                          mf.GetComponentInParent<MissionBase>()))
                    {
