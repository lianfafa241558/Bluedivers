...
public class RemoveUnderline: EditorWindow
{
    #region members
    private Vector2 scrollPos;
    private int charSize;
    ...
    #endregion
    #region get window
    [MenuItem("Tools/RemoveUnderline")]
    private static void OpenWindow()
    {
        GetWindow<RemoveUnderline>("RemoveUnderline").Show();
    }
    #endregion
    #region render the window
    private void OnGUI()
    {
        scrollPos = GUILayout.BeginScrollView(scrollPos);
        {
            ...
            GUI.skin.label.fontSize = 24;
            GUI.skin.label.alignment = TextAnchor.MiddleCenter;
            GUILayout.Label("Remove Underline");
            GUI.skin.label.fontSize = charSize;
            GUI.skin.label.alignment = TextAnchor.MiddleLeft;
            GUI.color = Color.white;
            GUILayout.Space(20);
    ...
    private void Rename(int count)
    {
        ...
        foreach (Object o in Selection.objects){
            ...
            if (Path.GetExtension(path_g) != "")//若后缀不为空, 则为assets文件夹物体
            {
                ...
                //AssetDatabase.RenameAsset(path_g, name.Replace("_", " "));//改名API
                if (!isAssetsObject)
                {
                        isAssetsObject = true;//修改flag
                }
