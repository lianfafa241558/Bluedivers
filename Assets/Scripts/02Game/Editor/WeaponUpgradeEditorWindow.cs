...
namespace Unity.FPS.Game.Editor
{
    ...
    /// <summary>
    /// 自定义 SO 选择弹窗，显示图标、名称、类型
    /// </summary>
    public class SOPickerPopup<T> : PopupWindowContent where T : ScriptableObject
    {
        private readonly List<T> _items;
        private readonly Action<T> _onPicked;
        private readonly Func<T, Sprite> _getIcon;
        private readonly Func<T, string> _getName;
        private readonly Func<T, string> _getType;
        private readonly Func<T, Color> _getTypeColor;
        private readonly Func<T, (string path, Color color)> _getFrame; // 可选的边框信息
        ...
        public SOPickerPopup(List<T> items, Action<T> onPicked,
            Func<T, Sprite> getIcon, Func<T, string> getName, Func<T, string> getType,
        ...
        public override Vector2 GetWindowSize()
        {
            ...
            var itemCount = Mathf.Min(filtered.Count, 12);
            var height = 28 + itemCount * (ItemHeight + 2) + 4;
            return new Vector2(320, Mathf.Min(height, MaxHeight));
        }
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            var listRect = new Rect(4, 26, rect.width - 8, rect.height - 30);
            var filtered = GetFiltered();
            var viewHeight = filtered.Count * (ItemHeight + 2);
            ...
            {
                var item = filtered[i];
                var itemRect = new Rect(0, i * (ItemHeight + 2), listRect.width - 16, ItemHeight);
                var hover = itemRect.Contains(Event.current.mousePosition);

                // 背景
                EditorGUI.DrawRect(itemRect, hover ? new Color(0.3f, 0.5f, 0.8f, 0.4f) : new Color(0.25f, 0.25f, 0.28f, 0.5f));
                ...
                GUI.Label(typeRect, _getType(item), typeLabelStyle);
