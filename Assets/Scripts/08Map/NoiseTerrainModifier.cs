...
//[ExecuteInEditMode]
public class NoiseTerrainModifier : MonoBehaviour
{
    ...
    [Foldout("巢穴", true)]
    [CustomLabel("巢穴概率")]
    public float nestProbability = 0.1f;
    [CustomLabel("巢穴尺寸")]
    public Vector2Int plateauSize = new(30,50);
    [CustomLabel("巢穴抬升强度")]
    public float nestIntensity = 0.1f;
    [CustomLabel("巢穴细节缩放")]
    public float nestScale = 100f;
    [CustomLabel("巢穴细节高度")]
    public float nestAmplitude = 0.1f;
    [CustomLabel("巢穴在边缘")]
    public bool nestEdge = false;
    [CustomLabel("巢穴预制体")]
    ...
    private List<(Vector2Int, int)> nests;

    //比如分辨率1024/宽512就是2
    private float mapscale=>terrain.terrainData.heightmapResolution/ terrain.terrainData.size.x;

    private int center => terrain.terrainData.heightmapResolution /2;

    private MapRoot m_mapRoot;
    void Awake()
    {
        ...
        m_mapRoot = GetComponent<MapRoot>();
    }
    /*
    private void OnValidate()
    {
        if(generateOnStart) ApplyFractalNoiseToTerrain();
    ...
    */
    ...
    public void ApplyFractalNoiseToTerrain()
    {
        ...
        m_mapRoot = GetComponent<MapRoot>();//测试时留着，正式时去掉
    ...
    /// 创建平台区域（要自行处理材质）
    /// </summary>
    void GeneratePlateaus()
    {
        // 10000是因为除2次100
        int plateaus = Mathf.FloorToInt(nestProbability * width * height / 10000f);//生成的数量
        int range = center-Constants.MapBorder/2;//尝试随机的范围



        nests=new();
        int count = 0;//进行循环的次数，最大100次
        for (int i = 0; i < plateaus&& count<100; i++,count++)
        {
            int px = Random.Range(center - range, center + range);//选中的点的x
            int py = Random.Range(center - range, center + range);//选中的点的y
            
            //int minHeight=999;
            int areaSize = Random.Range(plateauSize.x, plateauSize.y) / 2;//巢穴的尺寸
            int extra = (int)Mathf.Min(areaSize * 0.15f, 4);//额外延伸的范围

            //判断地图中心点到巢穴中心点的距离(防止超到地图外面)
            if (Vector2.Distance(new(px,py),new(center,center))>= center-(Constants.MapBorder/2 * mapscale) - plateauSize.y/2)
            {
                --i;continue;
            }
            Vector2Int point = new(px, py);
            //这样复杂度变成n平方了，很不好，应该开始就确定下来
            //巢穴起码相隔20m往上
            if (nests.Count>0 && nests.Select(item=> Vector2Int.Distance(item.Item1, point)).Min() < plateauSize.y+20)
            {
                --i; continue;
            }

            float plateauHeight = heightMap[px, py];
            //随机采样8个点确定平均高度
            for (int u = 0; u < 8; ++u)
            {
                int ux = Mathf.Clamp(px + Random.Range(areaSize, -areaSize), 0, width), uy = Mathf.Clamp(py + Random.Range(areaSize, -areaSize), 0, height);
                //minHeight = Mathf.Min(heightMap[ux, uy], minHeight);
                //plateauHeight = 0.5f*plateauHeight+0.5f* Mathf.Max(plateauHeight,);
                plateauHeight = 0.5f * plateauHeight + 0.5f * heightMap[ux, uy];
            }


            nests.Add((point,(int)(areaSize+extra)));
            plateauHeight += nestIntensity;
            //bool isCude = Tool.Random01();
            float offsetX = Random.Range(0f, 9999f);
            float offsetY = Random.Range(0f, 9999f);
            // 创建平台区域
            for (int y =Mathf.Max((int)(py - areaSize- extra),0); y < Mathf.Min((int)(py + areaSize+ extra), height) ; y++)
            {
                for (int x = Mathf.Max((int)(px - areaSize- extra), 0); x < Mathf.Min((int)(px + areaSize + extra), width) ; x++)
                {
                    float dist = Vector2.Distance(new Vector2(x, y), new Vector2(px, py));
                    if (dist < areaSize+ extra)
                    {
                        //影响力，距离平台越远越小
                        float influence = Mathf.Clamp01(1 - dist / (areaSize + extra));
                        
                        heightMap[x, y] = Mathf.Lerp(heightMap[x, y], plateauHeight,3*influence);
                        if (dist < areaSize) {
                            float influence2 = Mathf.Clamp01(1 - dist / areaSize);
                            float dx = offsetX + x / (float)width * nestScale;
                            float dy = offsetY + y / (float)height * nestScale;
                            heightMap[x, y] += (Mathf.PerlinNoise(dx, dy)*2-1) * nestAmplitude * influence2;
                        }

                        SetPixel(preHeight,x, y, 1, 2);
                    }
                }
            }
            ...
            for (int y = Mathf.Max((int)(py - areaSize - extra), 0); y < Mathf.Min((int)(py + areaSize + extra), height); y++)
            {
                for (int x = Mathf.Max((int)(px - areaSize - extra), 0); x < Mathf.Min((int)(px + areaSize + extra), width); x++)
                {
                    ...
                    if (dist < areaSize + extra)
                    {
                        float steepness = GetSteepness(x, y) / 90f;
                        //越陡峭的点，高度增高越多
                        heightMap[x, y] += steepness * 0.1f;

                        //影响力，距离平台越远越小
                        float influence = Mathf.SmoothStep(0,1,Mathf.Clamp01((areaSize + 0.2f*extra- dist) / extra));
                        //float steepness = GetSteepness(x, y) / 90f;
                        ...
                        // 沙地层（中等高度）
                        textureMap[x, y, 1] = Mathf.Clamp01(nowheight * 2f) * (1 - textureMap[x, y, 0]- influence);

                        // 侵蚀层（低洼区域）
                        textureMap[x, y, 2] = Mathf.Clamp01((1 - nowheight) * 2f) * (1 - textureMap[x, y, 0]- influence);

                        // 巢穴层
                        //textureMap[x, y, 3] = 1 - textureMap[x, y, 0]* influence;
                        textureMap[x, y, 3] = influence * (1 - textureMap[x, y, 4]);
            ...
            if (CreatPrefab)
            {
                float wsScale = terrain.terrainData.size.x / (float)width;
                int nestCount = (areaSize*2 / plateauSize.x)*2;

                //plateauSize.y / plateauSize.x = 4;

                Debug.LogWarning("最大巢穴" + plateauSize.y + "当前巢穴" + areaSize*2);
                Vector3 pos = terrain.transform.position + new Vector3(py * wsScale, 55, px * wsScale);



                for (int u = 1; u <= nestCount; ++u)
                {
                    var go = Instantiate(nestPrefab, m_mapRoot.unitRoot);
                    if (nestEdge)
                    {
                        go.transform.position = pos + RandomUtils.RandomVector3() * Random.Range(areaSize - 2, areaSize + 2) * wsScale;
                        go.transform.LookAt(pos);
                    }
                    else
                    {
                        float distance = (areaSize - 4) * Mathf.Sqrt(u / (float)nestCount); // 半径渐变
                        float angle = 2f * Mathf.PI / nestCount;

                        float x = Mathf.Cos(u*angle) * distance + Random.Range(-1, 1f);
                        float z = Mathf.Sin(u * angle) * distance + Random.Range(-1, 1f);
                        go.transform.position = pos + new Vector3(x, 0, z) * wsScale;
                        go.transform.eulerAngles = new(0, Random.Range(0, 360), 0);
                        //var scale = Random.Range(0, size - 2f);
                        //Debug.LogWarning("随机系数"+ scale+"最大"+ (size-2f));
                        //go.transform.position = pos+Tool.RandomVector3() * scale * wsScale;
                        //go.transform.eulerAngles=new(0,Random.Range(0,360),0);
                    }
                }
            }
    ...
    void GenerateEvacuationArea()
    {
        ...
        for (int i = 0; i < plateaus && count < 100; i++, count++)
        {
            ...
            //判断这个点到其他巢穴的距离
            //撤离点起码相隔30m往上
            if (nests.Count > 0 && nests.Select(item => Vector2Int.Distance(item.Item1, point)).Min() < plateauSize.y+30)
            {
                --i; continue;
            }
            ...
            for (int y = Mathf.Max((int)(py - areaSize - extra), 0); y < Mathf.Min((int)(py + areaSize + extra), height); y++)
            {
                for (int x = Mathf.Max((int)(px - areaSize - extra), 0); x < Mathf.Min((int)(px + areaSize + extra), width); x++)
                {
                    ...
                    if (dist < areaSize + extra)
                    {
                        ...
                        //textureMap[x, y, 3] = 1 - textureMap[x, y, 0]* influence;
                        //textureMap[x, y, 3] = influence * (1 - textureMap[x, y, 4]);
                        textureMap[x, y, 1] += influence * (1 - textureMap[x, y, 4]);

                    }
                }
            }
            //Debug.LogError("基础点" + new Vector3(point.x, plateauHeight, point.y) +"尺寸"+);
            StartPoint.transform.position = TerrainPosToWorldPos(new Vector3(point.x, plateauHeight, point.y));
            break;
    ...
    private void ModifityHeight()
    {
        for (int y = 0; y < height; y++)
        {
            ...
            {
                // 标准化高度
                heightMap[x, y] = heightMap[x, y] * 0.5f+0.5f;
            }
        }
    }
