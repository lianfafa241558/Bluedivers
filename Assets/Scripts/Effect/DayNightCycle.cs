...
public class DayNightCycle : MonoBehaviour
{
    ...
    [Header("=== 环境光颜色 ===")]
    public Color noonAmbient = new Color(1f, 0.96f, 0.88f);    // 正午环境光
    public Color sunsetAmbient = new Color(1f, 0.6f, 0.4f);    // 黄昏环境光
    public Color nightAmbient = new Color(0.15f, 0.2f, 0.35f);  // 夜晚环境光
    public Color dawnAmbient = new Color(0.8f, 0.9f, 1f);      // 黎明环境光
    ...
    private bool isNoon;
    Light sunLight;
    private Material useSkyboxMaterial;
    ...
    private void Awake()
    {
        ...
        useSkyboxMaterial = skyboxs[now.Hour % skyboxs.Length];
        RenderSettings.skybox = useSkyboxMaterial;
        // 如果需要更新光照，可以调用以下方法
        ...
        // 800取模，作为初始时间
        currentTime = (minuteSecond % Constants.FullDayDuration)/ Constants.FullDayDuration;
        //currentTime = 0.75f;
    ...
    IEnumerator SetDayState()
    {
        ...
        if (currentTime < dayStageTime[2] || currentTime > dayStageTime[5])
        {
            GlobalEventSub.DaySwitch(isNoon = false);
        }
        else
        {
            isNoon = true;
    ...
    private void Update()
    {
        colorValue = new Color[] { nightAmbient, sunsetAmbient, noonAmbient, noonAmbient, dawnAmbient, nightAmbient };
        // 更新时间进度
        currentTime += Time.deltaTime / Constants.FullDayDuration;
        ...
        if (GameRoot.GameState != Core.GameStateEnum.Game && GameRoot.GameState != Core.GameStateEnum.Bridge) return;

        if (isNoon && currentTime>= dayStageTime[5])
        {
            GlobalEventSub.DaySwitch(isNoon = false);
        }
        else if (!isNoon && currentTime > dayStageTime[2] && currentTime <= dayStageTime[5])
        {
            GlobalEventSub.DaySwitch(isNoon = true);
        }
    }
    ...
    private void UpdateSunRotation()
    {
        ...
        transform.rotation = Quaternion.Euler(angle%180, 0f, 0f);
    }
    ...
    void UpdateSkybox()
    {
        useSkyboxMaterial.SetFloat("_Lerp", LerpScale(skyValue));
    }
