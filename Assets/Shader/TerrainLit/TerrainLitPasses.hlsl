...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    //这可能有点冒险——这里的假设是，如果diffuseMap没有
    //alpha通道，然后diffAlbedo[n].a=1.0（和_DiffuseHasAlphaN=0.0）
    //在进入之前，_SmoothenessN实际上设置为max（_DiffuseHasAlphaN，_SmoonthenessN.）
    //这意味着如果我们有一个alpha通道，则_SmoothenessN被锁定为1.0，并且
    //否则，向下传递真实滑块值，并diffAlbedo[n].a==1.0。
...
// 方法功能：根据高度重新计算 4 层地形贴图的混合权重（让高处显示一种纹理，低处显示另一种）
// 参数：
//   splatControl：4 层地形的混合权重（r/g/b/a 分别对应 4 张地形贴图）
void HeightBasedSplatModify(inout half4 splatControl)
{
    // 高度信息储存在遮罩图的蓝色通道中
    // 我们用混合权重计算出【每层纹理的相对高度】
    half4 splatHeight = 0.5 * splatControl.rgba;

    // 找出 4 层纹理里【高度最高】的那一层
    half maxHeight = max(splatHeight.r, max(splatHeight.g, max(splatHeight.b, splatHeight.a)));

    // 确保【高度过渡值】不会为 0（避免计算错误）
    half transition = max(_HeightTransition, 1e-5);

    // 核心算法：
    // 把最高的一层设为基准，其他层根据高度差降低权重
    // 低于最高高度 - 过渡范围的部分，权重直接变成 0
    half4 weightedHeights = splatHeight + transition - maxHeight.xxxx;
    weightedHeights = max(0, weightedHeights);  // 所有负数全部变成 0

    // 给有效图层加一个极小值，避免权重全为 0（保证至少有一层纹理能显示）
...
{
    color.rgb *= color.a;
    //从技术上讲，我们不需要fogCoord，但它仍然是从顶点着色器传递的。
...
#endif
{
    ...
    if (_NumLayersCount <= 4)
        HeightBasedSplatModify(splatControl);
#endif
...
//如果使用延迟渲染
    ...
    // 定义PBR光照计算所需的BRDF数据结构体
    BRDFData brdfData;

    // 初始化BRDF数据
    // 参数：反照率、金属度、高光、高光强度、透明度、输出BRDF数据
    // 地形使用金属工作流，所以高光相关参数全部设为0
    InitializeBRDFData(albedo, 0, 0, 0, alpha, brdfData);

    // -------------------------- 烘焙光照计算 --------------------------
    half4 color;

    // 获取主光源（太阳光）信息，包含阴影、光照蒙版
    ...
    // -------------------------- 延迟渲染适配处理 --------------------------
    // 注释翻译：
    // 动态照明：通过缩放GBuffer材质属性来模拟SplatmapFinalColor()效果。
    // 这无法得到和正向渲染完全一致的结果，
    // 因为混合操作是在光照计算前执行，而不是光照后。

    // 将地形透明度混合应用到材质属性（适配延迟渲染）
    brdfData.albedo.rgb *= alpha;       // 反照率乘以透明度
    brdfData.diffuse.rgb *= alpha;      // 漫反射乘以透明度
    brdfData.specular.rgb *= alpha;     // 高光乘以透明度
    ...
    //间接照明（忽略复杂细节，保留平均色调）
    //half3 indirectResult = SampleSH(0)*indirectOcclusion; // 稳定环境光
    // 间接照明：使用随世界坐标连续变化的烘焙GI，而非固定的SampleSH(0)
    half3 indirectResult = inputData.bakedGI * indirectOcclusion; // 稳定环境光

    half originalStrength=0;
    ...
    half4 color = UniversalFragmentPBR(inputData, albedo, 0, 0, 0, occlusion,0, alpha);
    SplatmapFinalColor(color, inputData.fogCoord);
    return half4(color.rgb, 1.0h);
