...
namespace Meryuhi.Rendering
{
    ...
    public sealed class FullScreenFogRendererFeature : ScriptableRendererFeature
    {
        ...
        class FullScreenFogRenderPass : ScriptableRenderPass
        {
            ...
            private static readonly int HeightParamsShaderID = Shader.PropertyToID("_HeightParams");
            ...
            private void UpdateMaterial(FullScreenFog fog)
            {
                ...
                // Combined mode: pass linear height params to the shader.
                // x = startHeight, y = 1 / (startHeight - endHeight)
                if (FullScreenFog.UseHeightParams(mode))
                {
                    var heightDelta = fog.startHeight.value - fog.endHeight.value;
                    var invHeight = heightDelta == 0 ? float.MaxValue : 1 / heightDelta;
                    _passData.Material.SetVector(HeightParamsShaderID, new Vector4(fog.startHeight.value, invHeight, 0f, 0f));
                }
