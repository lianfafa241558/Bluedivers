...
namespace MCPForUnity.Editor.Clients.Configurators
{
    ...
    public class OpenClawConfigurator : McpClientConfiguratorBase
    {
        ...
        {
            "安装 OpenClaw",
            "安装桥接插件：npm install -g openclaw-mcp-bridge（或 pnpm add -g openclaw-mcp-bridge）",
            "在 MCP for Unity 中选择 OpenClaw 并点击「配置」",
            "OpenClaw 使用 MCP for Unity 当前选定的传输方式（HTTP 或 stdio）",
            "OpenClaw 通过 unityMCP__call 之类的代理工具访问 Unity MCP",
            "如果插件未热重载新配置，请重启 OpenClaw"
        };
