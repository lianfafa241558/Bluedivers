...
namespace MCPForUnity.Editor.Clients
{
    ...
    public abstract class McpClientConfiguratorBase : IMcpClientConfigurator
    {
        ...
        protected string GetUvxPathOrError()
        {
            ...
            {
                throw new InvalidOperationException("未找到 uvx。请安装 uv/uvx，或在「高级设置」中配置覆盖路径。");
            }
            return uvx;
        }
    ...
    public abstract class JsonFileMcpConfigurator : McpClientConfiguratorBase
    {
        ...
        public override string GetConfigureActionLabel()
            => client.status == McpStatus.Configured ? "注销" : "配置";
        ...
        public override IList<string> GetInstallationSteps() => new List<string> { "该客户端暂不提供配置步骤。" };
    }
    ...
    public abstract class ClaudeCliMcpConfigurator : McpClientConfiguratorBase
    {
        ...
        public override string GetConfigureActionLabel() => client.status == McpStatus.Configured ? "注销" : "配置";

        public override string GetConfigPath() => "由 Claude CLI 管理";
        ...
        {
            "确保已安装 Claude CLI",
            "使用「配置」添加 UnityMCP（或运行 claude mcp add UnityMCP）",
            "重启 Claude Code"
        };
