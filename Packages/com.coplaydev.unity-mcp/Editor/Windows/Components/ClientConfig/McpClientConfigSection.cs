...
namespace MCPForUnity.Editor.Windows.Components.ClientConfig
{
    ...
    public class McpClientConfigSection
    {
        ...
        private string GetStatusDisplayString(McpStatus status)
        {
            ...
            {
                McpStatus.NotConfigured => "未配置",
                McpStatus.Configured => "已配置",
                McpStatus.Running => "运行中",
                McpStatus.Connected => "已连接",
                McpStatus.IncorrectPath => "路径不正确",
                McpStatus.CommunicationError => "通信错误",
                McpStatus.NoResponse => "无响应",
                McpStatus.UnsupportedOS => "不支持的操作系统",
                McpStatus.MissingConfig => "缺少 MCPForUnity 配置",
                McpStatus.Error => "错误",
                McpStatus.VersionMismatch => "版本不匹配",
                _ => "未知",
            };
        }
        ...
        public void UpdateManualConfiguration()
        {
            ...
            {
                installationStepsLabel.text = "该客户端暂不提供配置步骤。";
            }
        }
        ...
        private void UpdateClaudeCliPathVisibility()
        {
            ...
            if (client is ClaudeCliMcpConfigurator)
            {
                ...
                if (string.IsNullOrEmpty(claudePath))
                {
                    ...
                    claudeCliPath.value = "未找到 — 点击“浏览”选择";
                }
                else
                {
                    claudeCliPathRow.style.display = DisplayStyle.Flex;
        ...
        private void UpdateClientProjectDirVisibility()
        {
            ...
            if (client is ClaudeCliMcpConfigurator)
            {
                ...
                {
                    clientProjectDirField.value = projectDir + "  （已覆盖）";
                }
                else
                {
        ...
        private void OnConfigureAllClientsClicked()
        {
            try
            {
                ...
                string headline = summary.SkippedCount > 0
                    ? $"已处理 {summary.SuccessCount + summary.FailureCount} 个检测到的客户端。（{summary.SkippedCount} 个未安装，已跳过。）"
                ...
                EditorUtility.DisplayDialog("配置检测到的客户端", message, "确定");
            ...
            {
                EditorUtility.DisplayDialog("配置失败", ex.Message, "确定");
            }
        }
        ...
        private void ConfigureClaudeCliAsync(IMcpClientConfigurator client)
        {
            ...
            ApplyStatusToUi(client, showChecking: true, customMessage: isCurrentlyConfigured ? "正在注销…" : "正在配置…");
            ...
            }).ContinueWith(t =>
            {
                ...
                {
                    errorMessage = t.Exception.GetBaseException()?.Message ?? "配置失败";
                }
            ...
            SkillSyncService.SyncAsync(installPath, branch, null, result =>
                {
                    ...
                    if (result.Success)
                    {
                        ...
                        EditorUtility.DisplayDialog("安装技能",
                            $"{summary}\n\n安装位置：{installPath}", "确定");
                    }
                    else
                    {
                        McpLog.Error($"SkillSync failed: {result.Error}");
                        EditorUtility.DisplayDialog("安装技能失败", result.Error, "确定");
                    }
                });
        }
        ...
        private void OnBrowseClaudeClicked()
        {
            ...
            string picked = EditorUtility.OpenFilePanel("选择 Claude CLI", suggested, "");
        ...
        private void OnBrowseProjectDirClicked()
        {
            ...
            string picked = EditorUtility.OpenFolderPanel("选择客户端项目目录", currentDir, "");
            if (!string.IsNullOrEmpty(picked))
            {
                if (!Directory.Exists(picked))
                {
                    EditorUtility.DisplayDialog("路径无效", "所选目录不存在。", "确定");
                    return;
                }
        ...
        private void OnOpenFileClicked()
        {
            ...
            try
            {
                if (!File.Exists(path))
                {
                    EditorUtility.DisplayDialog("打开文件", "配置文件路径不存在。", "确定");
                    return;
                }
        ...
        private void ApplyStatusToUi(IMcpClientConfigurator client, bool showChecking = false, string customMessage = null)
        {
            ...
            if (showChecking)
            {
                clientStatusLabel.text = customMessage ?? "检查中…";
            ...
            {
                clientStatusLabel.text = "传输方式不匹配";
                clientStatusIndicator.AddToClassList("warning");
            }
            else
            {
