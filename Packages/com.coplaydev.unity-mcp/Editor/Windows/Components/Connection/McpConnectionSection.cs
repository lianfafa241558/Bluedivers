    ...
    public class McpConnectionSection
    {
        ...
        private void InitializeUI()
        {
            ...
            if (httpUrlField != null)
                httpUrlField.tooltip = "MCP 服务器的 HTTP 终结点地址。本地服务器请使用 localhost。";
            if (unityPortField != null)
                unityPortField.tooltip = "Unity 内部 MCP 桥接套接字的端口，用于 stdio 传输。";
            if (connectionToggleButton != null)
                connectionToggleButton.tooltip = "开始或结束 Unity 与服务器之间的 MCP 会话。";
            ...
            if (apiKeyField != null)
            {
                ...
                apiKeyField.tooltip = "用于远程托管 MCP 服务器身份验证的 API 密钥";
        ...
        public void UpdateConnectionStatus()
        {
            ...
            if (isRunning)
            {
                ...
                connectionStatusLabel.text = $"会话已激活（{instanceName}）";
                statusIndicator.RemoveFromClassList("disconnected");
                statusIndicator.AddToClassList("connected");
                connectionToggleButton.text = stdioSelected ? "结束会话" : "断开连接";
            ...
            else
            {
                ...
                if (isStdioResuming)
                {
                    ...
                    statusIndicator.RemoveFromClassList("disconnected");
                    connectionToggleButton.text = stdioSelected ? "开始会话" : "连接";
                    connectionToggleButton.SetEnabled(false);
                }
                else
                {
                    connectionStatusLabel.text = "无会话";
                    statusIndicator.RemoveFromClassList("connected");
                    ...
                    {
                        connectionToggleButton.tooltip = "HTTP 远程模式需要 API 密钥，请在上方填写。";
                    }
                    else if (blockedByRemoteUrlPolicy)
                    {
                        connectionToggleButton.tooltip = remoteUrlError ?? "HTTP 远程地址已被当前安全设置阻止。";
                    }
                    else if (blockedByLocalUrlPolicy)
                    {
                        connectionToggleButton.tooltip = localUrlError ?? "HTTP 本地地址已被当前安全设置阻止。";
                    }
                    else
                    {
        ...
        public void UpdateHttpServerCommandDisplay()
        {
            ...
            if (!isLocalHttpUrlAllowed)
            {
                ...
                if (httpServerCommandHint != null)
                {
                    ...
                    httpServerCommandHint.text = $"⚠ {localUrlError ?? $"HTTP 本地需要回环地址（{requirements}）。"}";
            ...
            if (MCPServiceLocator.Server.TryGetLocalHttpServerCommand(out var command, out var error))
            {
                ...
                if (httpServerCommandHint != null)
                {
                    httpServerCommandHint.text = "如需手动启动服务器，请在你的终端中运行此命令。";
                }
                if (copyHttpServerCommandButton != null)
                {
            ...
            else
            {
                ...
                if (httpServerCommandHint != null)
                {
                    httpServerCommandHint.text = error ?? "当前配置下无法提供该命令。";
                }
                if (copyHttpServerCommandButton != null)
                {
                    copyHttpServerCommandButton.SetEnabled(false);
        ...
        private void UpdateStartHttpButtonState()
        {
            ...
            {
                startHttpServerButton.text = "启动中…";
            }
            else
            {
                startHttpServerButton.text = shouldShowStop ? "停止服务器" : "启动服务器";
            }
            ...
            startHttpServerButton.tooltip = httpLocalSelected
                ? (canStartLocalServer
                    ? string.Empty
                    : localUrlError ?? $"HTTP 本地需要回环地址（{HttpEndpointUtility.GetHttpLocalHostRequirementText()}）。")
                : string.Empty;
        }
        ...
        private async void OnHttpServerToggleClicked()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        string errorMsg = localPolicyError ?? "HTTP 本地地址已被当前安全设置阻止。";
                        EditorUtility.DisplayDialog("无法启动 HTTP 服务器", errorMsg, "确定");
            ...
            catch (Exception ex)
            {
                ...
                EditorUtility.DisplayDialog("错误", $"切换本地 HTTP 服务器失败：\n\n{ex.Message}", "确定");
            }
            finally
            {
        ...
        private async void OnConnectionToggleClicked()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        string errorMsg = remotePolicyError ?? "HTTP 远程地址已被当前安全设置阻止。";
                        EditorUtility.DisplayDialog("连接被阻止", errorMsg, "确定");
                    ...
                    else
                    {
                        ...
                        string errorMsg = state?.Error
                            ?? "启动 MCP 会话失败。请检查服务器地址以及服务器是否正在运行。";
                        EditorUtility.DisplayDialog("连接失败", errorMsg, "确定");
                        if (httpRemoteSelected)
                        {
            ...
            catch (Exception ex)
            {
                ...
                EditorUtility.DisplayDialog("连接错误",
                    $"切换 MCP 连接失败：\n\n{ex.Message}",
                    "确定");
            }
            finally
        ...
        private async void OnGetApiKeyClicked()
        {
            ...
            try
            {
                ...
                {
                    EditorUtility.DisplayDialog("API 密钥",
                        "此服务器不支持 API 密钥管理，请联系服务器管理员。",
                        "确定");
                    return;
                }
        ...
        public void UpdateTransportMismatchWarning(string clientName, ConfiguredTransport clientTransport)
        {
            ...
            if (hasMismatch)
            {
                ...
                transportMismatchText.text = $"⚠ {clientName} 配置为“{clientTransportName}”，但服务器设置为“{serverTransportName}”。" +
                    "请在“客户端配置”中点击“配置”以更新。";
        ...
        private static string TransportDisplayName(ConfiguredTransport transport)
        {
            return transport switch
            {
                ...
                ConfiguredTransport.Http => "HTTP 本地",
                ConfiguredTransport.HttpRemote => "HTTP 远程",
                _ => "未知"
            };
        }
    }
