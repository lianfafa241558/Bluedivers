...
namespace MCPForUnity.Editor.Windows.Components.Resources
{
    ...
    public class McpResourcesSection
    {
        ...
        public void Refresh()
        {
            ...
            if (!hasResources)
            {
                AddInfoLabel("未发现 MCP 资源。请为类添加 [McpForUnityResource] 特性以暴露资源。");
                UpdateSummary();
                return;
            }

            BuildCategory("内置资源", "built-in", allResources.Where(r => r.IsBuiltIn));

            var customResources = allResources.Where(r => !r.IsBuiltIn).ToList();
            if (customResources.Count > 0)
            {
                BuildCategory("自定义资源", "custom", customResources);
            }
            else
            {
                AddInfoLabel("已加载的程序集中未检测到自定义资源。");
            }
        ...
        private VisualElement CreateResourceRow(ResourceMetadata resource)
        {
            ...
            tagsContainer.Add(CreateTag(resource.IsBuiltIn ? "内置" : "自定义"));
