...
namespace MCPForUnity.Editor.Windows
{
    ...
    public class EditorPrefsWindow : EditorWindow
    {
        ...
        {
            var window = GetWindow<EditorPrefsWindow>("EditorPrefs 管理器");
        ...
        public void CreateGUI()
        {
            ...
            refreshButton.text = "↻";
            refreshButton.tooltip = "刷新 EditorPrefs";
            ...
            if (knownPrefTypes.TryGetValue(key, out var knownType))
            {
                ...
                switch (knownType)
                {
                    case EditorPrefType.Bool:
                        ...
                        item.Value = item.IsUnset ? "未设置。默认：False" : EditorPrefs.GetBool(key, false).ToString();
                        break;
                    case EditorPrefType.Int:
                        item.Type = EditorPrefType.Int;
                        item.Value = item.IsUnset ? "未设置。默认：0" : EditorPrefs.GetInt(key, 0).ToString();
                        break;
                    case EditorPrefType.Float:
                        item.Type = EditorPrefType.Float;
                        item.Value = item.IsUnset ? "未设置。默认：0" : EditorPrefs.GetFloat(key, 0f).ToString();
                        break;
                    case EditorPrefType.String:
                        item.Type = EditorPrefType.String;
                        item.Value = item.IsUnset ? "未设置。默认：（空）" : EditorPrefs.GetString(key, "");
                        break;
                }
            }
            else
        ...
        private void SaveValue(string key, string value, EditorPrefType type)
        {
            switch (type)
            {
                ...
                case EditorPrefType.Int:
                    ...
                    {
                        EditorUtility.DisplayDialog("错误", $"无法将 '{value}' 转换为 int", "确定");
                        return;
                    }
                    break;
                case EditorPrefType.Float:
                    ...
                    {
                        EditorUtility.DisplayDialog("错误", $"无法将 '{value}' 转换为 float", "确定");
                        return;
                    }
                    break;
                case EditorPrefType.Bool:
                    ...
                    {
                        EditorUtility.DisplayDialog("错误", $"无法将 '{value}' 转换为 bool（请使用 'True' 或 'False'）", "确定");
                        return;
                    }
                    break;
            }
