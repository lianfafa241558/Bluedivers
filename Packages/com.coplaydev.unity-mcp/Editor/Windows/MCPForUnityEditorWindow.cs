    ...
    public class MCPForUnityEditorWindow : EditorWindow
    {
        ...
        private void UpdateVersionLabel()
        {
            ...
            versionLabel.tooltip = AssetPathUtility.IsPreReleaseVersion()
                ? $"{ProductInfo.ProductName} v{version}（预发布包，使用预发布服务器通道）"
        ...
        private void ApplyUpdateCheckResult(UpdateCheckResult result, string currentVersion)
        {
            ...
            {
                updateNotificationText.text = $"有可用更新：v{result.LatestVersion}  （当前：v{currentVersion}）";
                updateNotificationText.tooltip = $"最新版本：v{result.LatestVersion}\n当前版本：v{currentVersion}";
        ...
        private static void BuildDependenciesSection(VisualElement container)
        {
            ...
            var desc = new Label("部分工具组需要可选包。安装后可解锁更多能力。");
            ...
            {
                if (!EditorUtility.DisplayDialog("安装全部依赖",
                    "将安装 Roslyn DLL、ProBuilder、Cinemachine、VFX Graph 和 glTFast。是否继续？",
                    "全部安装", "取消")) return;
                installAllButton.SetEnabled(false);
                installAllButton.text = "正在安装…";
            ...
            {
                if (!EditorUtility.DisplayDialog("卸载全部依赖",
                    "将移除 Roslyn DLL、ProBuilder、Cinemachine、VFX Graph 和 glTFast。是否继续？",
                    "全部卸载", "取消")) return;
                uninstallAllButton.SetEnabled(false);
                uninstallAllButton.text = "正在移除…";
                UninstallRoslyn();
                BatchUpmRemove(upmPackages, () =>
                {
                    uninstallAllButton.SetEnabled(true);
                    uninstallAllButton.text = "全部卸载";
                });
            });
            uninstallAllButton.text = "全部卸载";
            ...
            AddDependencyRow(content,
                "Roslyn（C# 12+ 编译器）",
                "为 execute_code 工具启用现代 C# 语法（scripting_ext 组）。",
                roslynLoaded,
                roslynInstalledLocally
                    ? "已通过 Plugins/Roslyn 安装 — execute_code 使用 Roslyn"
                    : "可用（从 NuGet/外部加载）— execute_code 使用 Roslyn",
                "未安装 — execute_code 回退到 C# 6（CodeDom）",
            ...
            AddDependencyRow(content,
                "ProBuilder",
                "manage_probuilder 工具所需（probuilder 组）。",
                hasProBuilder,
                "已安装",
                "未安装",
            ...
            AddDependencyRow(content,
                "Cinemachine",
                "为 manage_camera 增加虚拟相机支持（core 组）。",
                hasCinemachine,
                "已安装",
                "未安装 — 相机工具仍可使用",
            ...
            AddDependencyRow(content,
                "VFX Graph",
                "为 manage_vfx 工具启用 VisualEffect 支持（vfx 组）。",
                hasVfxGraph,
                "已安装",
                "未安装 — VFX 工具回退到 ParticleSystem/LineRenderer",
            ...
            AddDependencyRow(content,
                "glTFast（glTF/GLB 导入）",
                "为 AI 资产生成工具启用 .glb/.gltf 模型导入（asset_gen 组）。",
                hasGltfast,
                "已安装 — GLB 生成/导入可用",
                "未安装 — 无法导入 GLB；FBX 仍可用，或安装以启用 GLB",
            ...
            Action<Action> installAction, Action<Action> uninstallAction)
        {
            ...
            if (isInstalled && uninstallAction != null)
            {
                ...
                btn = new Button(() =>
                {
                    if (!EditorUtility.DisplayDialog("移除 " + name,
                        $"确定要移除 {name} 吗？", "移除", "取消")) return;
                    btn.SetEnabled(false);
                    btn.text = "正在移除…";
                    Action restore = () =>
                    {
        ...
        private static void BatchUpmAdd(string[] packageIds, Action onComplete = null)
        {
            ...
            EditorUtility.DisplayProgressBar("正在安装包", $"正在安装 {packageIds.Length} 个包…", 0.5f);
        ...
        private static void BatchUpmRemove(string[] packageIds, Action onComplete = null)
        {
            ...
            EditorUtility.DisplayProgressBar("正在移除包", $"正在移除 {packageIds.Length} 个包…", 0.5f);
