...
- **【必须】** 接口名以 `I` 前缀开头（带下划线），使用帕斯卡命名法，与项目主流约定一致（如`IRecyclable`、`IPhysical`、`IEquippable`、`IDrivable` ）。
  - 例外：历史代码中少量标准 `I_` 前缀接口（ `I_Entity`、`I_Actor`、`I_Damagable`、`I_GlobaManager`、`I_TickClass`等）暂不强制重构，但**新增接口必须使用 `I_` 前缀**。
...
- **【必须】** 明确指定所有成员的访问修饰符（`public`/`private`/`protected`/`internal`），不依赖默认值。
